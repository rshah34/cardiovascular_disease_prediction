...
Original file is located at
    https://colab.research.google.com/drive/1tlpbWcd4d0H8_AbZMYY-puzG7z3b4Tsv

# **CIS 5450 Final Project - Cardiovascular Disease Prediction**

*Sahishnu Hanumolu, Rohan Shah, Meghana Vasireddy*

# Part 1: Introduction

For our final project, we chose to discover the various health factors that most evidently are correlated with a positive case of Cardiovascular Disease. Going into the project, we intended to see how factors such as diet, exercise, health history, and even demographics can all be potential predictors of heart disease, which led us to want to build a model that would predict whether or not the person is at risk for heart disease with reasonable levels of sensitivity and specificity.

We used a dataset with a preselected set of variables specifically relevant to heart disease, and all of the data used has been taken from the Behavioral Risk Factor Surveillance System from 2021. Using the data presented to us, we sought to specifically view the data in comparison to the diagnosis of heart disease per patient, and then attempt to draw conclusions from there. We also utilized a dataset with additional numerical variables relevant from heart disease taken from the Behavioral Risk Factor Surveillance System from 2015.

The results are below, along with various takeaways, methodologies used to gather our findings, and conclusions after each section. Enjoy!
...
We used a dataset directly from Kaggle, and so we decided to use an API key directly from Kaggle and then create the directory for Kaggle so that we can directly download the dataset into our notebook. Upon doing so, we evaluated our dataframe and sought to understand the data through viewing its types and statistics.
...
We will first start by viewing our initial data now that we have completed loading it into our notebook.
"""
...
"""## 3.1 Inspecting Data
Upon viewing this data, we decided to specificially understand the different columns in the dataset and evaluate how we want to clean to optimize our findings.
"""
...
"""Evaluating these statistics gave us a few things to look further into:

*   First, we noticed that various categories within our dataset were of type "object" rather than a numerical data, and so we decided to encode our "object" categories as numerical values for the sake of visualization and understanding of responses by the people in the survey.

*   Secondly, we noticed that despite there being overlap in various pieces of data, we concluded that because each of these individual rows are from different people in a national survey, it would not make sense to remove any values from this data set. Also, since we have no null values in the set to begin with, we have no need to cover this issue.
...
"""**Approach:** To process the 2021 BRFSS dataset, we took inspiration from the 2015 BRFSS dataset. In order to work with classifier models, we wanted to ensure that all of our features were numeric. The 2015 BRFSS dataset actually has all numeric columns, where even non-continuous data, like general health ratings, are represented on an integer scale. Therefore, we decided to encode all of our categorical variables on an integer scale starting at 0 to maintain consistency with the past practices of the BRFSS dataset. Lastly, we decided to replace the Fruit_Consumption, Green_Vegetables_Consumption, and FriedPotato_Consumption features with a single diet column that is equal to the sum of Fruit_Consumption and Green_Vegetables_Consumption minus FriedPotato_Consumption in order to better capture the relationship between the three features.

For a more straightforward casing approach to manipulating our data, we utilized pandasql in order to perform this major step of cleaning, as shown below.
"""
...
Now that we have completed the analysis of our datasets, we decided to evaluate our data further through various visualizations. Utilizing various features of seaborn, matplotlib, and plotly, we created visuals such as countplots, barplots, interactive scatterplots, box plots, and a correlation heatmap in order to generate a stronger understanding of how we can use our data for our modeling.
"""
...
"""So based on this interpretation of the correlation matrix, we questioned the reasonability of further including height and weight given we have a BMI column; due to the high correlation between BMI and weight it made sense for us to remove the height and weight columns given that BMI is a measure of these two values. Therefore, we will remove these columns now."""
...
Logistic regression was our first choice for a baseline model because it is simple, interpretable, and quite honestly designed for binary classification problems. Especially in a healthcare setting, it is crucial to see which features influence the prediction, and logistic regression can help capture such aspects. Moreover, logistic regression uses linear relationships, so this approach serves as a benchmark to more complex model development.
"""
...
We used PCA to retrieve the components that explain 80% of the cumulative variance and passed that into a logistic regression model.
"""
...
We thought that a Random Forest model served as a great complement to logistic regression because it can handle complex, non-linear relationships in data, which is sometimes common in medicine. Moreover, Random Forest can pick up on interactions between different features, so this type of robustness can be particularly useful when it comes to features that have to do with lifestyle choices that are often related in some way.
"""
...
We chose a KNN model because it makes no assumptions about relationships in the underlying data. The model simply classifies based on the distance to existing points instead of trying to parameterize the features to arrive at some sort of decision criteria. Especially in a medical setting, we thought that such an algorithm could have some advantages in capturing more complex relationships that might not intuitively make sense when individually studying features.
"""
...
Here, we use the same models as the previous section, but we augment our training data with synthetic examples of the minority class. We thought that such an approach could have merits because our dataset does have considerable class imbalance with 56774 examples of no heart disease and only 4997 instances of individuals with heart disease. Therefore, generating synthetic examples of the minority class could help prevent the model from being overly biased toward outputting negatives in order to maximize accuracy.
...
Upon completing our modeling section, we have been able to make various takeaways from this project.


*   We first noticed that there exists a class imbalance in our data that has led to us seeing a relatively high accuracy without identifying actually positive cases, leading therefore to a very low recall. However, once we attempted to solve this issue through the usage of SMOTE, it was evident that despite a significant improvement in recall of true positive heart disease diagnoses, there also came a significantly increased number of false positives, thus bringing the accuracy of our models down.
*   Overall, this led us to conclude that working with this dataset proved very challenging to predict disese risk, meaning we may have required more specific features that could directly determine whether one is at risk for heart disese.

In the future, we realized that there are various things we'd reconsider when approaching this project.


*   We would first attempt to explore additional features that could enhance the prediction model itself, given the inaccuracies resulting from our current features.
*   Next, we would potentially consider integrating various other external datasets apart from strictly health-related issues, which may include but not be limited to environmental data and socioeconomic indicators of poor cardiovascular health.
*   Finally, we'd consider implementing a time-series analysis of data specifically on the lifestyle factors in order to capture specific trends or other temporal patterns contributing to either a positive or negative test of cardiovascular disease.

Thank you so much for reading through our project!
